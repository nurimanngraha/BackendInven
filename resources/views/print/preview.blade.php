...
<head>
    <meta charset="utf-8" />
    <title>Preview Cetak Aset {{ $payload }}</title>
    ...
    <div class="wrapper">
        ...
        <div class="qr-preview-area">
            <div class="qr-box" id="qrBox">
                {{-- SVG hasil generate pertama kali dari server --}}
                <div id="qrSvg">{!! $svg !!}</div>

                <div class="kode" id="qrCodeText">{{ $payload }}</div>

                <div class="aset-meta">
                    {{ $aset->nama_aset ?? $aset->nama ?? 'Aset' }}<br>
                    {{ $aset->merk_kode ?? $aset->merk ?? '' }}
                </div>
            </div>
        </div>
